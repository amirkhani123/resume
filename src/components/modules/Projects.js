        ...
        {data.map((item)=>(
          <div key={item.id} className="flex items-center flex-col  size-56 overflow-hidden rounded-lg  shadow-3xl dark:bg-white">
           ...
           <div className="flex items-center justify-between w-full p-2">
            <Link href={`${item.github}`} className=" flex items-center gap-2 px-2 py-3 rounded-lg bg-black text-white  my-hover hover:text-black hover:bg-white hover:border-black border">
            GitHub <FaGithub size={26}/>
            </Link>
            <Link href={`/${item.id}`} className="px-5 py-3 rounded-lg bg-green-500 text-white  my-hover hover:text-green-500 hover:bg-white hover:border-green-500 border">
            Details
            </Link>
           </div>
          </div>
